@extends('layouts.app')
@section('title', __('nav.import_staff'))
@section('content')
    <h1 class="mb-4 text-2xl font-bold tracking-tight text-slate-900">{{ __('nav.import_staff') }}</h1>
    <p class="mb-4 max-w-2xl text-sm text-slate-600">{{ __('imports.template_hint') }}</p>
    <a href="{{ route('admin.import.staff.template') }}" class="admin-btn-secondary mb-6">
        {{ __('imports.download_template') }} ({{ $excelSupported ? '.xlsx' : '.csv' }})
    </a>
    <p class="mb-4 max-w-2xl text-sm text-slate-600">{{ __('imports.staff_help') }}</p>
    @if(session('import_result'))
        ...
        <div class="admin-card mb-4 p-4 text-sm">
            ...
            @if(count($r['errors']))
                <ul class="mt-2 max-h-48 list-inside list-disc overflow-y-auto text-red-700">
    ...
    @endif
    <form method="post" action="{{ route('admin.import.staff.run') }}" enctype="multipart/form-data" class="admin-form-panel max-w-md">
        @csrf
