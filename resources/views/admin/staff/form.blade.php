...
@section('content')
    ...
    <form method="post" action="{{ $item->exists ? route('admin.staff.update', $item) : route('admin.staff.store') }}" class="admin-form-panel">
        ...
        <div><label class="block text-sm font-medium text-slate-700">{{ __('fields.college') }}</label>
            <select id="staff-form-college" name="college_id" class="admin-input" required>@foreach($colleges as $c)<option value="{{ $c->id }}" @selected(old('college_id', $item->college_id)==$c->id)>{{ $c->name_en }}</option>@endforeach</select>
        </div>
        <div><label class="block text-sm font-medium text-slate-700">{{ __('fields.department') }}</label>
            <select id="staff-form-department" name="department_id" class="admin-input" required>@foreach($departments as $d)<option value="{{ $d->id }}" @selected(old('department_id', $item->department_id)==$d->id)>{{ $d->name_en }}</option>@endforeach</select>
        </div>
    ...
    @push('scripts')
        @include('partials.admin-college-department-filter', [
            ...
            'collegeElementId' => 'staff-form-college',
            'departmentElementId' => 'staff-form-department',
